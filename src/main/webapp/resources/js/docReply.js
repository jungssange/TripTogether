...
var inHtml ="<fieldset>"+
			"<div class='txarea_wrap' style='float:left; width: 833px'>"+ 
			"<textarea id='re2contents' name='re2contents' rows='3' cols='75' onfocus='fn_memoFocus(this)' style='height:40px'></textarea>"+
			"</div>"+
			"<div class='btn_box'><a href='javascript:void(0);' style='width: 70px; background: url(images/common/btn_ok.gif) no-repeat 0px 0px; height: 35px;float:right;' onclick='fn_re2formSubmit();'></a></div>"+
			"</fieldset>";
...
function fn_ReplyModify(no, depth){
	...
	if(depth > 0) {
		$('#divReplyView'+ no + ' .txarea_wrap')[0].style.width = '770px'; /* 김준철: 대댓글 창 사이즈 수정 */
		$('#divReplyView'+ no + ' .btn_box')[0].style.top = '62px';
		$('#divReplyView'+ no + ' .btn_box')[0].style.right = '30px';
	} else {
		$('#divReplyView'+ no + ' .txarea_wrap')[0].style.width = '810px';
...
function chkMsgLength(objMsg) {
	...
	//st_len = document.getElementById('currentMsgLen');
	st_byte = document.getElementById('currentMsgByte');
	//steb_len = document.getElementById('currentMsgLenExceptBlank');
	//steb_byte = document.getElementById('currentMsgByteExceptBlank');
