(function() {
  ...
  callWithJQuery(function($) {
    ...
    Utilities
     */
	  var PivotData, addSeparators, aggregatorTemplates, aggregators, dayNamesEn, derivers, getSort, locales, mthNamesEn, naturalSort, numberFormat, pivotDataProcessor, pivotTableRenderer, renderers, sortAs, usFmt, usFmtInt, usFmtPct, zeroPad;
    ...
    aggregatorTemplates = {
      ...
      countPerPerson: function(formatter) {
        if (formatter == null) {
          formatter = usFmt;
        }
        return function() {
      ...
      sumOverNumber: function(formatter) {
          if (formatter == null) {
            formatter = usFmt;
          }
          return function(arg) {
	 ...
	 /*
	"건수": tpl.count(usFmtInt),
          "고유 값 수": tpl.countUnique(usFmtInt),
          "고유 값 목록": tpl.listUnique(", "),
          "합산": tpl.sum(usFmt),
          "정수 합": tpl.sum(usFmtInt),
          "평균": tpl.average(usFmt),
          "최소값": tpl.min(usFmt),
          "최대값": tpl.max(usFmt),
          "합을 통해 합계": tpl.sumOverSum(usFmt),
          "80 % 상한": tpl.sumOverSumBound80(true, usFmt),
          "80 % 하한": tpl.sumOverSumBound80(false, usFmt),
          "분수": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
          "행 분수": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
          "컬럼 분수": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
          "전체분수 합계": tpl.fractionOf(tpl.count(), "total", usFmtPct),
          "행 분수 합계": tpl.fractionOf(tpl.count(), "row", usFmtPct),
          "컬럼 분수 합계": tpl.fractionOf(tpl.count(), "col", usFmtPct)
    ...
    */
    ...
    pivotDataProcessor = function(opts) {
        var colAttrs, rowAttrs;
        colAttrs = opts.cols;
        rowAttrs = opts.rows;
        return function(pivotData, record) {
          var aggregator, allTotal, colKey, colKeys, colTotals, flatColKey, flatRowKey, l, len1, len2, n, ref, ref1, rowKey, rowKeys, rowTotals, tree, x;
          tree = pivotData.tree;
          ...
          colKey = [];
          rowKey = [];
          for (l = 0, len1 = colAttrs.length; l < len1; l++) {
            x = colAttrs[l];
            colKey.push((ref = record[x]) != null ? ref : "null");
          }
          for (n = 0, len2 = rowAttrs.length; n < len2; n++) {
            x = rowAttrs[n];
          ...
          if (colKey.length !== 0 && rowKey.length !== 0) {
            ...
            return tree[flatRowKey][flatColKey].push(record);
          }
        };
      };
    ...
    PivotData = (function() {
      function PivotData(input, opts) {
        ...
        this.processor = opts.processor(opts);
        ...
        PivotData.forEachRecord(input, opts.derivedAttributes, (function(_this) {
          return function(record) {
            if (opts.filter(record)) {
            	return _this.processor(_this, record);
            }
          };
        })(this));
      }
      ...
      try {
        ...
        fn = function(c) {
          ...
          $("<p>").appendTo(valueList).append($("<button>", {
            ...
            type: "button"
          }).text("닫기").bind("click", closeFilter));
        ...
        $("<td>").addClass('pvtVals').appendTo(tr1).append(aggregator).append();
        ...
        refreshDelayed = (function(_this) {
          return function() {
            ...
            subopts.processor = opts.processor;
